 ...
 * AI Agent System
 *
 * AI-powered agent for generating learning roadmaps and modules.
 * Uses multiple AI providers (Groq, Cerebras, Brave) for reliable inference.
 *
 * Key Responsibilities:
 * - Generate comprehensive learning roadmaps from user topics
 * - Create sequential modules with clear objectives
 * - Research and include official documentation links
 * - Generate practical challenges with verification criteria
 * - Provide starter code and ground truth examples
 *
 * Dependencies:
 * - AI client from ./ai (Groq, Cerebras, Brave)
 * - GenerateResponse schema for type safety
 *
 * @module apps/web/lib/agents
 */
...
/**
 * Interface for roadmap briefing generated by AI
 */
...
/**
 * Main agent system for generating learning roadmaps.
 * Uses AI to create structured, sequential learning paths with
 * modules, challenges, documentation, and verification criteria.
 *
 * @example
 * ```typescript
 * const agent = new RoadmapAgentSystem({ cerebras: 'api-key' });
 * const roadmap = await agent.generatePro('React Hooks');
 * console.log('Modules:', roadmap.modules);
 * ```
 */
export class RoadmapAgentSystem {
  ...
  /**
   * Generates a complete learning roadmap for the given topic.
   * Creates a briefing, then generates all modules in parallel.
   *
   * @param topic - The learning topic to generate a roadmap for
   * @returns Promise resolving to GenerateResponse with modules, challenges, and verification criteria
   *
   * @example
   * ```typescript
   * const roadmap = await agent.generatePro('Solidity Smart Contracts');
